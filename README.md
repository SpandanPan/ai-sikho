...
## Product detail pages + agent-drafted descriptions

Every pack card ("GenAI Engineer Starter Pack", etc.) is now clickable,
landing on `/pack/[slug]/[variant]` — a real description, price, and an
"Add to cart" button, plus a "Do you also want to try" cross-sell block
(`CrossSell.tsx`) suggesting one other pack, one paid course, and
mentoring. Standard e-commerce pattern (every product page has some
version of this) — someone already committed to buying is the easiest
person to sell a second, related thing to.

The descriptions themselves were generated by the local Ollama agent
(`generateProductDescription` in `contentAgent.ts`,
`scripts/generate-product-descriptions.ts`), not hand-written — Starter
Packs and fluency courses in plain language, Kits and technical courses
in more technical language, per the actual prompt sent to the model. **No
MCP or web-search tool access** — this app has no search API wired up
(no key available in this environment), so the agent draws only on its
own knowledge, not a live lookup. Reviewed by hand before landing in
`interviewPacks.ts`/`courses.ts`, same standard as every other AI-drafted
content here.

## AI Pulse: sidebar, not footer

Moved from a horizontal ticker near the bottom of the page to a sticky
vertical sidebar on the left (desktop, `PulseFeed.tsx`'s `orientation="vertical"`,
auto-scrolling `scrollTop` instead of `scrollLeft`), staying visible while
you scroll the rest of the page instead of being one more section to
scroll past. Mobile has no room for a persistent sidebar, so the original
horizontal ticker is kept as a fallback there (`orientation="horizontal"`,
hidden on `lg+`) — same data, same auto-scroll preference, two
presentational variants of one component.

## Other polish

- **Logo**: `Logo.tsx` — a small "network node" SVG mark (three connected
  nodes) instead of a plain letter tile, using the existing theme color
  tokens so it re-themes in dark mode automatically. Clicking it (or the
  wordmark) always goes home — this already worked, just confirmed.
- **Nav order**: sign-in/profile now sits to the right of the cart icon,
  not before it.
- **Topic-based pack animations** (`TrackIcon.tsx`): a blinking cursor for
  GenAI Engineer, stepping dots for Agentic AI, growing bars for Data
  Scientist, a flowing dot for Data Engineering — all `motion-safe:` (see
  `globals.css`), so `prefers-reduced-motion: reduce` disables every one
  automatically.
- **Articles archive**: `Article.archived` — evergreen pieces that stay
  accurate for a long time get their own section on `/articles`, separate
  from newer/current-affairs pieces at the same level. Nothing's marked
  archived yet (no articles are published at all today), but the section
  exists for when content moves there.
- **Soft-gate timer**: 60s → 150s before the sign-in nudge — more room to
  actually explore before being asked to sign in.
- **Hero image**: shrunk (420px → 260/300px min-height) — it was taking
  up more of the first screen than its actual content justified.
