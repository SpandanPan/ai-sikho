...
// fetch-news cron job). No sample/placeholder fallback content — an empty
// or failed fetch shows a plain "checking in" state instead of fake news,
// so nothing here is ever mistaken for a real headline.
//
...
export default function PulseFeed({ orientation = "horizontal" }: { orientation?: "horizontal" | "vertical" }) {
  ...
  const statusLabel = status === "live" ? "Live" : status === "loading" ? "Loading" : "Checking in";
  ...
  if (orientation === "vertical") {
    return (
      <section className="w-full">
        ...
        {status !== "live" ? (
          <p className="text-xs text-ink-soft border border-paper-line rounded bg-paper-raised p-3">
            {status === "loading" ? "Fetching today's AI news…" : "No fresh stories right now — check back soon."}
          </p>
        ) : (
          <div
            ref={trackRef}
  ...
  return (
    <section id="pulse" className="border-b border-paper-line py-9">
      ...
      {status !== "live" ? (
        <p className="text-sm text-ink-soft border border-paper-line rounded bg-paper-raised p-4">
          {status === "loading" ? "Fetching today's AI news…" : "No fresh stories right now — check back soon."}
        </p>
      ) : (
